...
def retreino_triagem():
    ...
    def promover_modelo(export_info: dict, metrics: dict) -> str:
        ...
        for filename in MODEL_FILES.values():
            # copyfile, not copy/copy2: both of those also chmod the destination,
            # and chmod requires owning the file. Under a bind mount the artifacts
            # belong to uid 0 while this process runs as uid 50000, so the write
            # succeeds and the chmod raises EPERM. Only the bytes matter here --
            # metadata.json already carries version and trained_at as provenance.
